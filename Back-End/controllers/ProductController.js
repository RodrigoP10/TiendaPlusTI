...
function saveProduct(req, res) {
    ...
    if (adminID != req.admin.sub || req.admin.role != 'ADMINISTRADOR') {
        ...
        if (params.name && params.description && params.stock && params.category && params.price && params.productCode) {
...
function deleteProduct(req, res) {
    ...
    Product.findByIdAndRemove(productID, deleted, (err, productDeleted) => {
        if (err) {
            ...
            if (!productDeleted) {
                res.status(200).send({ message: 'No se ha podido eliminar el producto' });
            } else {
